//= react 모듈 중에서 useState만 골라서 불러오겠다/-> {}를 따로 쓸 때에
import { useState } from "react";

function Exam01(){
    //데이터
    //- const는 값이 변하지 않는 변수를 의미
    // - react에서는 화면을 자동으로 갱신해주기 때문에 불변값이 아니면 처리를 안해준다
    ...
    return (
        ...
        {/* 데이터를 출력 */}
        <h2>number = {number}</h2>
        {/* (주의) class를 부여할 때는 class가 아니라 className이라고 적어야 함 */}
        <button className="btn btn-primary ms-2" onClick={plus}>+</button>
